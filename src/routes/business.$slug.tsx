  ...
  return (
    <>
      ...
      <section className="relative h-[60vh] min-h-[480px] overflow-hidden">
        <img src={business.image} alt={business.title} className="absolute inset-0 h-full w-full object-cover" />
          ...
          <Container>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              <div className="flex items-center gap-3 text-gold">
                <span className="grid h-12 w-12 place-items-center rounded-xl bg-white/10 backdrop-blur"><Icon size={20} /></span>
                <span className="text-xs font-semibold uppercase tracking-[0.25em]">{business.tagline}</span>
              </div>
              <h1 className="mt-4 max-w-3xl font-display text-4xl text-white md:text-6xl">{business.title}</h1>
        ...
        <Container>
          ...
          <Stagger className="mt-16 grid gap-6 md:grid-cols-3">
            {business.achievements.map((a) => (
              <motion.div variants={itemVariants} key={a.label} className="rounded-2xl border border-border bg-card p-8 text-center shadow-soft">
                <div className="font-display text-5xl text-gold-gradient">{a.value}</div>
                <div className="mt-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">{a.label}</div>
              </motion.div>
            ))}
          </Stagger>
        </Container>
      ...
      <section className="bg-mist py-24">
        <Container>
          <Reveal><SectionHeading eyebrow="Services" title={`What ${business.title} offers`} /></Reveal>
          <Stagger className="mt-12 grid gap-6 md:grid-cols-2">
            {business.services.map((s) => (
              <motion.div variants={itemVariants} key={s.title} className="flex gap-4 rounded-2xl border border-border bg-card p-6 shadow-soft">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-gold/15 text-gold"><Check size={18} /></div>
                <div>
      ...
      <section className="bg-navy-gradient py-20 text-white">
        <Container>
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
            <div>
              <h3 className="font-display text-3xl text-white md:text-4xl">Partner with {business.title}</h3>
              <p className="mt-2 max-w-xl text-white/70">Talk to our team about partnerships, supply, franchising or careers in this business unit.</p>
            </div>
          ...
          <div className="mt-16 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {BUSINESSES.filter((b) => b.slug !== business.slug).map((b) => (
              <Link key={b.slug} to="/business/$slug" params={{ slug: b.slug }}
